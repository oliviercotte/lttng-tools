	...
	struct consumer_output *output)
{
	...
	case CONSUMER_DST_NET:
	{
		...
		if (output->dst.net.control_isset &&
			...
			ret = uri_to_str_url(&output->dst.net.control, uri,
				PATH_MAX);
			...
			ret = uri_to_str_url(&output->dst.net.data, uri,
				PATH_MAX);
			if (ret < 0) {
				ret = LTTNG_ERR_INVALID;
				goto end_net_output;
	...
	struct snapshot *snapshot)
{
	...
	rcu_read_lock();
	cds_lfht_for_each_entry(snapshot->output_ht->ht, &iter.iter, node,
		node) {
	...
	struct lttng_save_session_attr *attr, lttng_sock_cred *creds)
{
	...
	size_t session_name_len = strlen(session->name);
	...
	if (lttng_save_session_attr_get_configuration_url(attr)) {
		const char *provided_path =
			lttng_save_session_attr_get_configuration_url(attr);
	...
	/*
	 * Check the path fits in PATH_MAX, including the / and trailing .lttng
	 * extension.
	 */
	if (len + session_name_len + 8 > PATH_MAX) {
		ret = LTTNG_ERR_SET_URL;
		goto end;
	}
	...
	lttng_sock_cred *creds)
{
	...
	if (session_name) {
		...
		cds_list_for_each_entry(session, &list->head, list) {
			...
			/*
			 * Don't abort if we don't have the required
			 * permissions.
			 */
